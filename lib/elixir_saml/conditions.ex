defmodule ElixirSAML.Conditions do
	...
	"""
	def verify_date(%ElixirSAML.Conditions{ not_before: not_before, not_on_or_after: not_on_or_after }, now \\ DateTime.utc_now()) do
	...
	"""
	def verify_audience(%ElixirSAML.Conditions{ audience_restrictions: audience_restrictions } = conditions, audience) do
		case audience in audience_restrictions do
			true -> {:ok, conditions}
			false -> {:error, "Audience is not in restricted audiences"}
		end
	end
	...
	def parse(xml) do
		with \
			{:ok, not_before, _} = xpath(xml, ~x"//Conditions/@NotBefore") |> to_string |> DateTime.from_iso8601(),
			{:ok, not_on_or_after, _} = xpath(xml, ~x"//Conditions/@NotOnOrAfter") |> to_string |> DateTime.from_iso8601(),
			audience_restrictions = xpath(xml, ~x"//Conditions/AudienceRestrictionCondition/Audience/text()"l) |> Enum.map(&to_string(&1))
		do
			{:ok, %ElixirSAML.Conditions{
